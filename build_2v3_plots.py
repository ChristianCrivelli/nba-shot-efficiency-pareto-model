...
data/season_totals_all.csv (produced by season_totals.py).
  ...
  of all time since the 3-point line began (1979-80+), colored by season
  to show how the frontier has drifted over eras.
...
def _iso_efficiency_line(ax, lo, hi):
    ...
    ax.text(hi, hi, "  EV(3) = EV(2)", color=INK_MUTED, fontsize=8.5,
            ha="left", va="center", rotation=0)


def plot_current_era(df: pd.DataFrame, season: str, min_fga: int, seed: int) -> Path:
...
def plot_historic(df: pd.DataFrame, start_season: str) -> Path:
...
def main():
    ...
    parser.add_argument("--historic-start", default="1979-80", help="first season eligible for the historic top-100")
    ...
    p1 = plot_current_era(df, args.season, args.min_fga, args.sample_seed)
    print(f"Saved: {p1}")

    p2 = plot_historic(df, args.historic_start)
    print(f"Saved: {p2}")
